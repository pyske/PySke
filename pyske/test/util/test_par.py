...
def test_distribution_is_valid():
    ...
    distr = [4, 12, 27, 18]
    size = sum(distr)
    res = par.Distribution(distr).is_valid(size)
    parallel.NPROCS = tmp
    assert res
...
def test_distribution_is_not_valid():
    ...
    distr = [4, 12, 27, 18]
    size = sum(distr)
    res = par.Distribution(distr).is_valid(size - 1)
    assert not res
...
def test_distribution_balanced_valid():
    ...
    distr = par.Distribution.balanced(size)
...
def test_distribution_pid_of_index_0():
    ...
    distr = par.Distribution([4, 12, 27, 18])
...
def test_distribution_pid_of_index_with_empty_1():
    # pylint: disable=missing-docstring
    value = random.randint(0, 100)
    distr = par.Distribution([0, 10, 0, 20])
    res = distr.to_pid(9, value)
    exp = 1, (9, value)
    assert exp == res
